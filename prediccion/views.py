...
def predecir_lana():
    ...
    y = data[['kilos_lana']]
    resultado_prediccion = np.round(modelo_lana.predict(X), 2)
    r2 = 'Exactitud de la predicción (R2): {:.2%}'.format(
        r2_score(y, resultado_prediccion))
...
def predecir_corderos():
    ...
    y = data[['cordero/as']]
    resultado_prediccion = np.round(modelo_corderos.predict(X), 0)
    r2 = 'Exactitud de la predicción (R2): {:.2%}'.format(
        r2_score(y, resultado_prediccion))
...
def predecir_finura():
    path = staticfiles_storage.path(
        'data/modelo_prediccion_lana_finura.pickle')
    ...
    mse = 'Error (MSE): {:.2f} micras'.format(
        mean_squared_error(datos_test['finura'], predicciones))
    ...
    finura = {
        ...
        "data": json.dumps({
            ...
            "y_prediccion": [round(valor, 2) for valor in predicciones],
...
def predecir_rinde():
    ...
    mse = 'Error (MSE): {:.2f} rinde'.format(
        mean_squared_error(datos_test.rinde, prediccion))
    ...
    rinde = {
        ...
        "data": json.dumps({
            "label_prediccion": [2018, 2019, 2020],
            "label_real": build_years(datos_train.fecha),
            "y_prediccion": [round(valor, 2) for valor in prediccion],
